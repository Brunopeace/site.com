...
// Obtém o token do Firebase Cloud Messaging (FCM)
...
function verificarClientesVencendo() {
    const hoje = new Date();
    
    // Exemplo: suponha que os clientes estejam armazenados no localStorage
    const clientes = JSON.parse(localStorage.getItem("clientes")) || [];
    
    clientes.forEach(cliente => {
        const dataVencimento = new Date(cliente.vencimento); // Data de vencimento do cliente
        const doisDiasAntes = new Date(dataVencimento);
        doisDiasAntes.setDate(dataVencimento.getDate() - 2);

        // Se hoje for igual à data de aviso, enviar notificação
        if (hoje.toDateString() === doisDiasAntes.toDateString()) {
            enviarNotificacao(cliente.nome);
        }
    });
}

// Chamar essa função uma vez ao dia
setInterval(verificarClientesVencendo, 24 * 60 * 60 * 1000);

async function enviarNotificacao(nomeCliente) {
    const token = await obterTokenFCM();
    if (!token) return console.error("🚫 Nenhum token FCM encontrado.");

    const notificacao = {
        to: token,
        notification: {
            title: "Cliente Próximo do Vencimento",
            body: `O cliente ${nomeCliente} vence em 2 dias.`,
    ...
    fetch("https://fcm.googleapis.com/fcm/send", {
        method: "POST",
        headers: {
            "Authorization": "key=xUQdCgm6XOmePOFD30kFZSCvMlKJsPrkm_u0BtSMIu4", // Server Key do Firebase Cloud Messaging
    ...
    .catch(err => console.error("⚠️ Erro ao enviar notificação:", err));
}
